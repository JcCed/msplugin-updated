...
class MS_Init_ImportProcess():
    ...
    # Use the following script to look for the inputs of the Principled BSDF node:
    # for key in bpy.data.materials["REPLACE_WITH_MATERIAL_NAME_HERE"].node_tree.nodes["Principled BSDF"].inputs: 
    #     print(key)
    ...
    def CreateMaterial(self):
        ...
        self.mat.node_tree.nodes[self.parentName].inputs["Specular Tint"].default_value = (0, 0, 0, 1) # Set Specular Tint
        self.mat.node_tree.nodes[self.parentName].inputs["Coat Tint"].default_value = (0, 0, 0, 1)
    ...
    def CreateTextureMultiplyNode(self, aTextureType, bTextureType, PosX, PosY, aPosX, aPosY, bPosX, bPosY, aColorspace, bColorspace, connectToMaterial, materialInputIndex):
        ...
        self.mat.node_tree.links.new(multiplyNode.inputs["A"], textureNodeA.outputs["Color"])
        self.mat.node_tree.links.new(multiplyNode.inputs["B"], textureNodeB.outputs["Color"])

        if connectToMaterial:
            self.ConnectNodeToMaterial(materialInputIndex, multiplyNode, 2)
    ...
    def ConnectNodeToMaterial(self, materialInputIndex, textureNode, textureNodeInputIndex = 0):
        self.mat.node_tree.links.new(self.nodes.get(self.parentName).inputs[materialInputIndex], textureNode.outputs[textureNodeInputIndex])
